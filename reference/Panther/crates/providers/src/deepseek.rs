...
impl ProviderInterface for DeepSeekProvider {
    async fn chat(&self, request: LLMRequest) -> PantherResult<LLMResponse> {
        if self.api_key.is_empty() {
            return Err(PantherError::LLMError("DeepSeek API key not configured".into()));
        }
        ...
        if !resp.status().is_success() {
            ...
            return Err(PantherError::LLMError(format!("DeepSeek returned {}: {}", status, body)));
        }

        let parsed: OAIResponse = resp
            .json()
            .await
            .map_err(|e| PantherError::LLMError(format!("DeepSeek parse error: {}", e)))?;

        parse_oai_response(parsed, LLMProvider::DeepSeek)
    }
}
