...
using LLama.Native.Pure;

namespace LLama.Native
{
    /// <summary>
    /// A reference to a set of llama model weights (native or pure C# backend).
    /// </summary>
    ...
    public sealed class SafeLlamaModelHandle
        : SafeLLamaHandleBase
    {
        internal PureModelState? _pureState;

        /// <summary>
        /// Get the rope (positional embedding) type for this model
        /// </summary>
        public LLamaRopeType RopeType => _pureState != null ? LLamaRopeType.None : llama_model_rope_type(this);

        /// <summary>
        /// The number of tokens in the context that this model was trained for
        /// </summary>
        public int ContextSize => _pureState != null ? _pureState.NCtxTrain : llama_model_n_ctx_train(this);

        /// <summary>
        /// Get the rope frequency this model was trained with
        /// </summary>
        public float RopeFrequency => _pureState != null ? 1f : llama_model_rope_freq_scale_train(this);

        /// <summary>
        /// Dimension of embedding vectors
        /// </summary>
        public int EmbeddingSize => _pureState != null ? _pureState.NEmbd : llama_model_n_embd(this);

        /// <summary>
        /// Get the size of this model in bytes
        /// </summary>
        public ulong SizeInBytes => _pureState != null ? _pureState.SizeInBytes : llama_model_size(this);

        /// <summary>
        /// Get the number of parameters in this model
        /// </summary>
        public ulong ParameterCount => _pureState != null ? _pureState.ParameterCount : llama_model_n_params(this);

        /// <summary>
        /// Get the number of layers in this model
        /// </summary>
        public int LayerCount => _pureState != null ? _pureState.NLayer : llama_model_n_layer(this);

        /// <summary>
        /// Get the number of heads in this model
        /// </summary>
        public int HeadCount => _pureState != null ? _pureState.NHead : llama_model_n_head(this);

        /// <summary>
        /// Get the number of KV heads in this model
        /// </summary>
        public int KVHeadCount => _pureState != null ? _pureState.NHeadKv : llama_model_n_head_kv(this);

        /// <summary>
        /// Get the number of SWA in this model
        /// </summary>
        public int SWACount => _pureState != null ? 0 : llama_model_n_swa(this);

        /// <summary>
        /// Returns true if the model contains an encoder that requires llama_encode() call
        /// </summary>
        public bool HasEncoder => _pureState != null ? false : llama_model_has_encoder(this);

        /// <summary>
        /// Returns true if the model contains a decoder that requires llama_decode() call
        /// </summary>
        public bool HasDecoder => _pureState != null ? true : llama_model_has_decoder(this);

        /// <summary>
        /// Returns true if the model is recurrent (like Mamba, RWKV, etc.)
        /// </summary>
        public bool IsRecurrent => _pureState != null ? false : llama_model_is_recurrent(this);
        
        /// <summary>
        /// Returns true if the model is diffusion based (like LLaDA , Dream etc )  
        /// </summary>
        public bool IsDiffusion => _pureState != null ? false : llama_model_is_diffusion(this);
        ...
        public string Description
        {
            ...
            {
                if (_pureState != null)
                    return "Pure C# backend (LLamaSharp)";
                unsafe
                {
        ...
        /// </summary>
        /// <returns></returns>
        public int MetadataCount => _pureState != null ? _pureState.MetadataStrings.Count : llama_model_meta_count(this);
        ...
        /// <summary>
        /// Create a handle that wraps the pure C# model state (no native DLL).
        /// </summary>
        internal static SafeLlamaModelHandle FromPure(PureModelState state)
        {
            var h = new SafeLlamaModelHandle();
        ...
        {
            if (NativeLibraryConfig.UsePureCSharpBackend)
            {
                var state = PureBackend.LoadModel(modelPath, ref lparams);
                return FromPure(state);
            }
        ...
        public uint TokenToSpan(LLamaToken token, Span<byte> dest, int lstrip = 0, bool special = false)
        {
            if (_pureState != null)
            {
                int len = _pureState.Tokenizer.TokenToPiece((int)token, dest, lstrip, special);
                return (uint)Math.Abs(len);
            }
        ...
        public LLamaToken[] Tokenize(string text, bool addBos, bool special, Encoding encoding)
        {
            if (_pureState != null)
            {
                var ids = _pureState.Tokenizer.Encode(text, addBos, special, encoding);
                var tokens = new LLamaToken[ids.Length];
                for (int i = 0; i < ids.Length; i++)
                    tokens[i] = (LLamaToken)ids[i];
                return tokens;
            }
        ...
        {
            if (_pureState != null)
                return _pureState.MetadataStrings;
        ...
        public sealed class Vocabulary
        {
            ...
            internal unsafe LLamaVocabNative* VocabNative => _model._pureState != null
                ? throw new InvalidOperationException("Pure C# backend: vocabulary native access not available.")
            ...
            public int Count
            {
                get
                {
                    if (_model._pureState != null)
                        return _model._pureState.VocabSize;
                    unsafe
                    {
            ...
            public LLamaVocabType Type
            {
                ...
                {
                    if (_model._pureState != null)
                        return LLamaVocabType.BytePairEncoding;
                    unsafe
                    {
