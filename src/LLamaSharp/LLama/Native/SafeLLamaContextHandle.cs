...
{
    /// <summary>
    /// A safe wrapper around a llama_context (native or pure C# backend).
    /// </summary>
        ...
        : SafeLLamaHandleBase
    {
        ...
        internal PureContextState? _pureContextState;

        /// <summary>
        /// Total number of tokens in the context
        /// </summary>
        public uint ContextSize => _pureContextState != null ? _pureContextState.ContextSize : llama_n_ctx(this);
        ...
        /// </summary>
        public uint BatchSize => _pureContextState != null ? (uint)Math.Min(512, _pureContextState.ContextSize) : llama_n_batch(this);

        /// <summary>
        /// Get the physical maximum batch size for this context
        /// </summary>
        public uint UBatchSize => _pureContextState != null ? BatchSize : llama_n_ubatch(this);
        
        /// <summary>
        /// Get the number of maximum sequences allowed
        /// </summary>
        public uint MaxSeq => _pureContextState != null ? 1u : NativeApi.llama_n_seq_max(this);
        ...
        {
            get => _pureContextState != null ? 1 : llama_n_threads(this);
            set { if (_pureContextState == null) llama_set_n_threads(this, value, BatchThreads); }
        }
        ...
        {
            get => _pureContextState != null ? 1 : llama_n_threads_batch(this);
            set { if (_pureContextState == null) llama_set_n_threads(this, GenerationThreads, value); }
        }

        /// <summary>
        /// Get the pooling type for this context
        /// </summary>
        public LLamaPoolingType PoolingType => _pureContextState != null ? LLamaPoolingType.None : llama_pooling_type(this);
        ...
        {
            if (_pureContextState != null)
            {
                _pureContextState.Dispose();
                _pureContextState = null;
        ...
        internal static SafeLLamaContextHandle FromPure(PureContextState state, SafeLlamaModelHandle model)
        {
            var ctx = new SafeLLamaContextHandle();
            ctx.SetHandle((IntPtr)1);
            ctx._pureContextState = state;
            ctx._model = model;
            var success = false;
        ...
        {
            if (model._pureState != null)
            {
                var pureCtx = PureBackend.CreateContext(model._pureState, lparams);
                return FromPure(pureCtx, model);
            }
        ...
        public Span<float> GetLogits(int numTokens = 1)
        {
            ...
            if (_pureContextState != null)
            {
                if (numTokens <= 0)
                    return default;
                return _pureContextState.GetLogitsSpanLast(numTokens);
            }
        ...
        public Span<float> GetLogitsIth(int i)
        {
            ...
            if (_pureContextState != null)
            {
                var span = _pureContextState.GetLogitsSpan(i);
                if (span.IsEmpty)
            ...
            if (_pureContextState != null)
            {
                lock (GlobalInferenceLock)
                {
                    using (batch.ToNativeBatch(out var nb))
                    {
                        unsafe
                        {
                            var tokSpan = new ReadOnlySpan<LLamaToken>(nb.tokens, nb.n_tokens);
                            var posSpan = new ReadOnlySpan<LLamaPos>(nb.pos, nb.n_tokens);
                            var logitsSpan = new ReadOnlySpan<byte>(nb.logits, nb.n_tokens);
                            _pureContextState.Decode(tokSpan, posSpan, logitsSpan);
                        }
                        return DecodeResult.Ok;
                    }
                }
