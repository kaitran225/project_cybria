...
export function diffDocuments(a, b) {
    ...
    for (const id of bLayerIds) {
        if (!aLayerIds.has(id))
            diff.layersAdded.push(id);
    }
    for (const id of aLayerIds) {
        if (!bLayerIds.has(id))
            diff.layersRemoved.push(id);
    }
    ...
    for (const [layerId, bLayer] of bLayerMap) {
        ...
        const removed = (aLayer?.pixels ?? []).filter((p) => !bPixels.has(pixelKey(p)));
        if (added.length > 0)
            diff.pixelsAdded[layerId] = added;
        if (removed.length > 0)
            diff.pixelsRemoved[layerId] = removed;
    }
