...
export function fingerprint(doc: PixelAssetDocument): AssetFingerprint {
  const paletteHash = hashString(
    [...doc.palette].sort((a, b) => a.id - b.id).map((p) => p.hex).join(",")
  );
  ...
  const paletteUsage = doc.palette.map((p) =>
    totalPixels > 0 ? (usedCounts.get(p.id) ?? 0) / totalPixels : 0
  );
