import type { DomainRule } from "@pixode/asset-core";
import type { PixelStyle } from "./types.js";

export function maxColorsRule(style: PixelStyle): DomainRule {
  return (ctx) => {
...
export function paletteComplianceRule(style: PixelStyle): DomainRule {
  return (ctx) => {
...
export function outlineRule(style: PixelStyle): DomainRule {
  return (ctx) => {
    ...
    for (const [li, layer] of ctx.doc.layers.entries()) {
      ...
      const pixelSet = new Set(
        layer.pixels.map((p) => `${p.x},${p.y}`)
      );
      const outlineRoleIds = new Set(
        ctx.doc.palette
      ...
      for (const px of layer.pixels) {
        if (outlineRoleIds.has(px.c)) continue;
        ...
        for (const [nx, ny] of neighbors) {
          ...
          if (!pixelSet.has(`${nx},${ny}`)) {
            const hasOutlineNeighbor = layer.pixels.some(
              (op) => op.x === px.x && op.y === px.y && outlineRoleIds.has(op.c)
            );
...
export function canvasSizeRule(style: PixelStyle): DomainRule {
  return (ctx) => {
...
export function createStyleRules(style: PixelStyle): DomainRule[] {
  return [
    maxColorsRule(style),
