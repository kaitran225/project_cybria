...
export const defaultSettings = {
  ...
  pianorollView: false,
  replWidgetViz: true,
  replWidgetSpectrum: true,
  replWidgetPiano: true,
  maxPolyphony: 128,
...
export const $settings = computed(settingsMap, (state) => {
  ...
  return {
    ...
    masterVolume:
      state.masterVolume === undefined || state.masterVolume === ''
        ? 1
        : Math.max(0, Math.min(1, Number(state.masterVolume))),
    pianorollView: parseBoolean(state.pianorollView),
    replWidgetViz: parseBoolean(state.replWidgetViz ?? true),
    replWidgetSpectrum: parseBoolean(state.replWidgetSpectrum ?? true),
    replWidgetPiano: parseBoolean(state.replWidgetPiano ?? true),
...
export const setMasterVolume = (volume) =>
  settingsMap.setKey('masterVolume', Math.max(0, Math.min(1, Number(volume))));
export const setPianorollView = (enabled) => settingsMap.setKey('pianorollView', !!enabled);
export const setReplWidgetViz = (enabled) => settingsMap.setKey('replWidgetViz', !!enabled);
export const setReplWidgetSpectrum = (enabled) => settingsMap.setKey('replWidgetSpectrum', !!enabled);
export const setReplWidgetPiano = (enabled) => settingsMap.setKey('replWidgetPiano', !!enabled);
