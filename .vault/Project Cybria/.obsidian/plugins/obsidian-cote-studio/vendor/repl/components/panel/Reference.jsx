...
export const Reference = memo(function Reference() {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden">
      <div
        className={cx(
          'h-full flex flex-col w-1/3',
          embedded ? 'border-r border-[var(--border)]' : 'text-foreground border-r border-muted',
        )}
      >
        <div className={embedded ? 'cote-shell-tab-panel__search' : 'w-full flex'}>
          <Textbox
            className={embedded ? undefined : 'w-full border-0 border-b border-muted'}
        ...
        {referenceTag && referenceTag !== 'all' && (
          <div className="px-2 py-1">
            <button
              type="button"
              className={embedded ? 'studio-btn secondary studio-btn--compact' : 'text-foreground border border-muted border-t-0 border-l-0 px-1 py-0.5 my-2 cursor-pointer font-sans'}
        ...
        <div className={embedded ? 'cote-shell-tab-panel__body' : 'h-full p-2 overflow-y-auto bg-opacity-50'}>
          <div className={embedded ? 'cote-shell-list' : undefined}>
            {searchVisibleFunctions.map((entry) => (
              <Fragment key={`entry-${entry.name}`}>
                <button
                  type="button"
                  className={cx(
                    embedded ? 'cote-shell-list__item' : 'whitespace-nowrap cursor-pointer hover:opacity-50 text-ellipsis block text-left w-full',
                    entry.name === selectedFunction &&
                      (embedded ? 'cote-shell-list__item--active' : 'bg-lineHighlight font-bold'),
                  )}
                  onClick={() => {
                    setSelectedFunction(entry.name === selectedFunction ? null : entry.name);
                  }}
                >
                  <span className={embedded ? 'cote-shell-list__title' : undefined}>{entry.name}</span>
                  {entry.synonyms ? (
                    <span className={embedded ? 'cote-shell-list__meta' : 'opacity-50 text-sm'}>
                      {entry.synonyms?.join(', ')}
                    </span>
                  ) : null}
                </button>
      ...
      <div
        className={cx(
          'w-2/3 break-normal flex-col overflow-y-auto overflow-x-hidden p-4 flex relative',
          embedded && 'cote-shell-settings',
        )}
        id="reference-container"
      >
        <div className={embedded ? 'cote-shell-reference' : 'prose dark:prose-invert min-w-full px-1 text-sm'}>
          {!embedded && <h2>API Reference</h2>}
          {embedded ? (
            <p className="cote-shell-reference__intro">
              Functions you can use in patterns. You do not need to memorize all of them — a small set is enough to
              start making music.
            </p>
          ) : (
          ...
          <div className={embedded ? 'cote-shell-reference__tags' : undefined}>
              ...
              .map(([t, count]) => (
                <span key={t}>
                  <button
                    type="button"
                    className={cx(
                      embedded ? 'studio-btn secondary studio-btn--compact cote-shell-reference__tag-btn' : 'select-none text-white border border-muted px-1 py-0.5 my-2 cursor-pointer text-sm/8 no-underline font-sans',
                      referenceTag === t && (embedded ? '' : 'bg-muted text-foreground'),
                    )}
                    onClick={() => toggleTag(t)}
                  >
                    {t}&nbsp;({count})
          ...
          {detailVisibleFunctions.map((entry, i) => (
            <section key={i} className={embedded ? 'cote-shell-reference__section' : 'font-sans'}>
              <div className={embedded ? 'cote-shell-reference__header' : 'flex flex-row items-center mt-8 justify-between'}>
                <h3
                  className={embedded ? 'cote-shell-reference__heading' : 'font-mono my-0 pt-4'}
                  id={`doc-${entry.name}`}
                >
                  {entry.name}
                </h3>
                {entry.tags && (
                  <span className={embedded ? 'cote-shell-reference__tag' : 'ml-2 text-xs text-foreground border border-muted px-1 py-0.5'}>
                    {entry.tags.filter((t) => !ignoredTags.includes(t)).join(', ')}
                  </span>
                )}
              </div>
              {!!entry.synonyms_text && (
                <p className={embedded ? 'cote-shell-reference__synonyms' : undefined}>
                  Synonyms: <code>{entry.synonyms_text}</code>
                </p>
              )}

              <div
                className={embedded ? 'cote-shell-reference__desc' : undefined}
                dangerouslySetInnerHTML={{ __html: entry.description }}
              />
              <ul className={embedded ? 'cote-shell-reference__params' : undefined}>
              ...
              {entry.examples?.map((example, j) => (
                <pre className={embedded ? 'cote-shell-reference__example' : 'bg-background border border-muted'} key={j}>
                  {example}
                </pre>
              ))}
            </section>
          )) || <p className={embedded ? 'cote-shell-reference__intro' : 'font-sans'}>Search or select a tag to get started.</p>}
