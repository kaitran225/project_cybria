...
function Checkbox({ label, value, onChange, disabled = false }) {
  const embedded = useCoteEmbedded();
  if (embedded) {
    return <ShellCheckbox label={label} value={value} onChange={onChange} disabled={disabled} />;
  }
  return (
...
function SelectInput({ value, options, onChange }) {
  const embedded = useCoteEmbedded();
  if (embedded) {
    return <ShellSelect value={value} options={options} onChange={onChange} />;
  }
  return (
...
function FormItem({ label, children, sublabel }) {
  ...
  if (embedded) {
    return (
      <ShellFormItem label={label} sublabel={sublabel}>
        {children}
      </ShellFormItem>
    );
  }
  ...
  return (
    <div
      className={cx(
        ...
        embedded ? 'cote-shell-settings p-4 space-y-4' : 'p-4 text-foreground space-y-4',
      )}
    >
      ...
      {!embedded ? (
        <>
          <FormItem label="Editor syntax theme">
            <SelectInput
...
function PrebakeSettingsContent() {
  const { prebakeScript } = useSettings();
  const embedded = useCoteEmbedded();
  ...
  return (
    <div
      className={cx(
        ...
        embedded ? 'cote-shell-settings' : 'text-foreground',
      )}
    >
      <div
        className={cx(
          'flex flex-col grow overflow-hidden h-full',
          embedded ? 'border border-[var(--border)] rounded-[var(--radius-sm)]' : 'bg-background',
        )}
      >
        <section
      ...
      <div
        className={cx(
          'flex justify-end items-center px-4 whitespace-nowrap',
          embedded ? 'cote-shell-tab-panel__toolbar' : 'border-t border-muted',
        )}
      >
...
export function SettingsTab({ started }) {
  ...
  const embedded = useCoteEmbedded();
  return (
    <div className={cx('w-full h-full flex flex-col overflow-hidden', embedded ? 'cote-shell-tab-panel' : 'text-foreground')}>
      <div
        className={cx(
          embedded ? 'cote-shell-tab-panel__toolbar' : 'px-2 shrink-0 h-8 space-x-4 flex max-w-full overflow-x-auto border-b border-muted',
        )}
      >
        <ButtonGroup
          wrap
