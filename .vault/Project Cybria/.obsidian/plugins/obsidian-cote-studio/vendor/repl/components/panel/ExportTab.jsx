...
import { ShellButton, ShellCheckbox, ShellFormItem } from './ShellForms.jsx';
...
function FormItem({ label, children, disabled }) {
  const embedded = useCoteEmbedded();
  if (embedded) {
    return <ShellFormItem label={label}>{children}</ShellFormItem>;
  }
  return (
...
export default function ExportTab(Props) {
  ...
  return (
    <>
      <div className={embedded ? 'cote-shell-settings p-4 space-y-4 w-full' : 'text-foreground w-full space-y-4 p-4'}>
        ...
        {embedded ? (
          <ShellButton
            label={exporting ? 'Exporting…' : 'Export to WAV'}
            variant="primary"
            disabled={exporting}
            ...
            onClick={async () => {
              ...
              await handleExport(startCycle, endCycle, sampleRate, maxPolyphony, multiChannelOrbits, downloadName)
                .then(() => {
                  document.getElementById('exportProgressModal')?.close();
                })
                .finally(() => {
                  setExporting(false);
                  setProgress(0);
